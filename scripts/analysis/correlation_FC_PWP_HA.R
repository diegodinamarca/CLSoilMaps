...
p = ggplot(data, aes(x = obs, y = sim))+
  ...
  geom_label(data = metrics, aes(label = message), x = 0.2, y = 0.7, size = 2.2)+
...
p = ggplot(data, aes(x = obs, y = sim))+
  geom_point(alpha = 0.7)+
  geom_label(data = metrics, aes(label = message), x = 0.1, y = 0.4, size = 2.2)+
  geom_smooth(method = "lm", se = T)+
  geom_abline(slope = 1, intercept = 0)+
  tune::coord_obs_pred()+
...
# Available Moisture (HA)
fname = here("proc","correlations","HA_correlation.csv")
data = read_csv(fname) %>% 
  mutate(depth = factor(depth, levels = depths))
data = data %>% filter(obs <= 0.75)
...
p = ggplot(data, aes(x = obs, y = sim))+
  ...
  geom_abline(slope = 1, intercept = 0)+
  tune::coord_obs_pred()+
  labs(x = "observed", y = "simulated", title = "Available Moisture")+
  facet_wrap(depth~.)
plot(p)
ggsave(filename = here("results","figures","HA_correlation_r2.png"), width = 7, height = 5, plot = p)
