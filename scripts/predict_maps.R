...
for (v in 2:2){
  ...
  for (s in c(3)){
    ...
    cargar_covs <- function(depth){
      ...
      # #Bandas espectrales
      # f <- list.files(path = "./materiales/Landsat/IMG",pattern = ".tif",full.names = T);f
      # print("bandas L8")
      #
      # bands <- brick(f)
      # names(bands) <- c('blue', 'green', 'red', 'nir', 'swir1', 'swir2')
      ...
      names(depth.r) = "depth"
      # ae <- der_topo[[1]]
      # lapply(1:nlayers(climate), function(c){
      #   print(names(climate[[c]]))
      #   compareRaster(climate[[c]], ae)
      # })
      #
      # lapply(1:nlayers(texturas11x11), function(c){
      #   print(names(texturas11x11[[c]]))
      #   compareRaster(texturas11x11[[c]], ae)
      # })
      #
      # lapply(1:nlayers(texturas21x21), function(c){
      #   print(names(texturas21x21[[c]]))
      #   compareRaster(texturas21x21[[c]], ae)
      # })
      #
      # lapply(1:nlayers(texturas51x51), function(c){
      #   print(names(texturas51x51[[c]]))
      #   compareRaster(texturas51x51[[c]], ae)
      # })
      #
      # lapply(1:nlayers(texturas81x81), function(c){
      #   print(names(texturas81x81[[c]]))
      #   compareRaster(texturas81x81[[c]], ae)
      # })
      #
      # print("geo")
      # compareRaster(ae, geo)
    ...
    # write_csv(as.data.frame(names(covars)), "./materiales/covariables.csv")
    names(covars)
    ...
    if (!((length(cov.names)) == nlayers(covars))) cat("Problem matching covariates with their files \n")
    
    
    # # add depth as covariate
    # depth.file <- str_c("./materiales/depths/depth_",depths[s],".tif");depth.file
    # covars <- c(covars, depth.file)
    ...
    # index for removing existing bootstrapmaps
    bootstrap.files <- list.files(res.dir.depth)
    
    index_rf <- str_split(bootstrap.files, "Bootstrap") %>% unlist %>% str_split(".tif") %>% unlist %>% as.integer %>% .[!is.na(.)];index_rf
    index_qrf <- str_split(bootstrap.files, "Bootstrap") %>% unlist %>% str_split("_") %>% unlist %>% as.integer %>% .[!is.na(.)] %>% unique;index_qrf
    ...
    if (model == "RF"){
      ...
      # avgmap <- rast(paste0(res.dir,var_name,"_mean.tif"))
    ...
    # 
    # if (model == "QRF"){
    #   res0.05 <- stack(list.files(path = res.dir.depth, pattern = "q0.05", full.names = TRUE))
    #   res0.95 <- stack(list.files(path = res.dir.depth, pattern = "q0.95", full.names = TRUE))
    #   
    #   llim_map <- mean(res0.05)
    #   writeRaster(llim_map, filename = paste0(res.dir,var_name,"_Llim.tif"), overwrite = T)
    #   
    #   ulim_map <- mean(res0.95)
    #   writeRaster(ulim_map, filename = paste0(res.dir,var_name,"_Ulim.tif"), overwrite = T)
    #   
    #   range_map <- ulim_map - llim_map
    #   writeRaster(range_map, filename = paste0(res.dir,var_name,"_PIRange.tif"), overwrite = T)
