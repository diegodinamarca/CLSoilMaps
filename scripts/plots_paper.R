...
arcilla <- read_csv(files[1])
arena <- read_csv(files[2])
da <- read_csv(files[3])
limo <- read_csv(files[4])
...
# exportar
write_csv(arcilla, "./materiales/Database/Arcilla_0_5cm.csv")
write_csv(arena, "./materiales/Database/arena_0_5cm.csv")
write_csv(da, "./materiales/Database/Da_0_5cm.csv")
write_csv(limo, "./materiales/Database/Limo_0_5cm.csv")

####################### FIN PARTE 1 ######################
#####-----------------------------------------------######
####################### PARTE 2: PUNTOS POR UNIDAD GEOMORFOLOGICA ###################################
...
####################### FIN PARTE 2 ######################
#####-----------------------------------------------######
############# PARTE 3: PUNTOS POR UNIDAD GEOMORFOLOGICA Y ZONA CLIMATICA ###############################################################
...
######################## FIN PARTE 3 #####################
#####-----------------------------------------------######
############# PARTE 4: DISTRIBUCION DE VALORES PREDICHOS POR UGEO Y ZONA CLIMATICA ################################################################
...
######################## FIN PARTE 4 #####################
#####-----------------------------------------------######
############# PARTE 5: METRICAS DE DISTRIBUCION POR UGEO Y ZONA CLIMATICA ##########
# area de estudio para la projeccion
ae <- rast("./materiales/area_estudio/ae_buffer_0.001deg.tif")
...
bd %>% group_by(Fuente) %>% summarise(n = n()) %>% 
  write_csv("./plots/Puntos_por_fuente.csv")
...
bd.p %>% filter(value != -999) %>%
  ...
  write_csv(file = "./materiales/Database/BD_para_cartografia.csv")
...
dir <- "./resultados/mapas_pred/post_procesado/100m"
list.files(dir)
...
ggsave(plot = g, filename = "./CR2SOILS_100M_5-15cm.png", width = 16, height = 12)
